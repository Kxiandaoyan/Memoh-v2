...
func NewProvidersHandler(log *slog.Logger, service *providers.Service, modelsService *models.Service, accountService *accounts.Service) *ProvidersHandler {
...
func (h *ProvidersHandler) requireAdmin(c echo.Context) error {
...
func (h *ProvidersHandler) Create(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("create provider failed", slog.Any("error", err))
...
func (h *ProvidersHandler) List(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("list providers failed", slog.Any("error", err))
...
func (h *ProvidersHandler) Get(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("get provider failed", slog.String("id", id), slog.Any("error", err))
...
func (h *ProvidersHandler) ListModelsByProvider(c echo.Context) error {
	...
	if err != nil {
		...
		h.logger.Error("list models by provider failed", slog.String("id", id), slog.Any("error", err))
		return echo.NewHTTPError(http.StatusNotFound, "provider or models not found")
	}
	return c.JSON(http.StatusOK, resp)
}
...
func (h *ProvidersHandler) GetByName(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("get provider by name failed", slog.String("name", name), slog.Any("error", err))
		return echo.NewHTTPError(http.StatusNotFound, "provider not found")
	}
...
func (h *ProvidersHandler) Update(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("update provider failed", slog.String("id", id), slog.Any("error", err))
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to update provider")
	}
...
func (h *ProvidersHandler) Delete(c echo.Context) error {
	...
	if err := h.service.Delete(c.Request().Context(), id); err != nil {
		h.logger.Error("delete provider failed", slog.String("id", id), slog.Any("error", err))
...
func (h *ProvidersHandler) Count(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("count providers failed", slog.Any("error", err))
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to count providers")
	}
