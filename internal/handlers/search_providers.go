...
func NewSearchProvidersHandler(log *slog.Logger, service *searchproviders.Service, accountService *accounts.Service) *SearchProvidersHandler {
...
func (h *SearchProvidersHandler) requireAdmin(c echo.Context) error {
...
func (h *SearchProvidersHandler) Create(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("create search provider failed", slog.Any("error", err))
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to create search provider")
	}
...
func (h *SearchProvidersHandler) List(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("list search providers failed", slog.Any("error", err))
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to list search providers")
	}
...
func (h *SearchProvidersHandler) Get(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("get search provider failed", slog.String("id", id), slog.Any("error", err))
		return echo.NewHTTPError(http.StatusNotFound, "search provider not found")
	}
...
func (h *SearchProvidersHandler) Update(c echo.Context) error {
	...
	if err != nil {
		h.logger.Error("update search provider failed", slog.String("id", id), slog.Any("error", err))
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to update search provider")
	}
	return c.JSON(http.StatusOK, resp)
}
...
func (h *SearchProvidersHandler) Delete(c echo.Context) error {
	...
	if err := h.service.Delete(c.Request().Context(), id); err != nil {
		h.logger.Error("delete search provider failed", slog.String("id", id), slog.Any("error", err))
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to delete search provider")
	}
