...
**后台模型分流：** 在 Bot 设置中可配置「后台模型 (Background Model)」，用于心跳、定时任务、子代理等后台场景。未设置时自动回退到对话模型。

| 任务类型 | 使用模型 | 典型场景 |
|----------|----------|----------|
| 用户对话 | 对话模型 (Chat Model) | 实时交互 |
| 心跳维护 | 后台模型 → 对话模型 | 周期性自检、记忆整理 |
| 定时任务 | 后台模型 → 对话模型 | Cron 触发的自动化 |
| 子代理执行 | 后台模型 → 对话模型 | 后台分工任务 |

**成本效果：** 后台任务通常不需要顶级模型。用 GPT-4o-mini（$0.15/M input）替代 GPT-4o（$2.50/M input）处理心跳和调度，可降低后台 Token 成本约 94%，同时不影响用户对话质量。

配合三级提示词（full/minimal/micro）和智能记忆门控，整体 Token 消耗可比未优化系统低 60-80%。
