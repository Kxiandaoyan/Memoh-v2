...
- [概念指南](#概念指南)
  ...
  - [配置步骤](#配置步骤)
  - [人设与文件的关系](#人设与文件的关系)
  - [OpenViking 分层上下文数据库](#openviking-分层上下文数据库)
  - [心跳与子智能体](#心跳与子智能体)
...
> **推荐首选：[Qwen3 Embedding 8B](https://openrouter.ai/qwen/qwen3-embedding-8b)** — Qwen 家族最新 Embedding 模型，继承了强大的多语言能力、长文本理解和推理能力。在文本检索、代码检索、文本分类、聚类等多项任务上表现领先。支持 32K 上下文，通过 OpenRouter（`https://openrouter.ai/api/v1`）即可使用，价格仅 $0.01/百万 token。
...
普通的向量记忆是"平面"的 —— 每段记忆平等地存储和检索。但人类的记忆是分层的：有些是核心知识（总是需要的），有些是详细细节（偶尔查阅的）。OpenViking 模仿了这种结构：
...
1. Bot 对话时，先加载 L0 摘要（极少 token）了解全貌
2. 根据需要，按需加载 L1/L2 的特定部分
3. 大幅减少每次对话的 token 消耗，同时保留完整的知识深度

**如何启用：**

1. 在 Bot 的"人设"页面，开启 **"启用 OpenViking 上下文数据库"** 开关
2. 系统自动生成 `ov.conf` 配置文件，并从你已配置的 Provider/模型中预填 API 信息
3. Bot 容器内即可使用 OpenViking Python API 进行知识管理

> OpenViking 需要 Embedding 模型支持。推荐使用 [Qwen3 Embedding 8B](https://openrouter.ai/qwen/qwen3-embedding-8b) 或本地 Embedding 模型。
