...
for migration_file in $(ls "$MIGRATIONS_DIR"/*.down.sql 2>/dev/null | sort -r -t/ -k2 -V); do
    if [ -f "$migration_file" ]; then
        if ! $PSQL_CMD -f "$migration_file" > /dev/null 2>&1; then
            echo -e "${RED}Error: Drop failed - $(basename "$migration_file")${NC}"
            exit 1
        fi
        echo -e "${GREEN}  Reverted: $(basename "$migration_file")${NC}"
    fi
done

$PSQL_CMD -c "DROP TABLE IF EXISTS schema_migrations;" > /dev/null 2>&1
