...
for migration_file in $(ls "$MIGRATIONS_DIR"/*.up.sql 2>/dev/null | sort); do
    if [ -f "$migration_file" ]; then
        ...
        already_applied=$($PSQL_CMD -t -c "SELECT COUNT(*) FROM schema_migrations WHERE version = '$version';" 2>/dev/null | tr -d ' ')
        ...
        $PSQL_CMD -c "INSERT INTO schema_migrations (version) VALUES ('$version') ON CONFLICT DO NOTHING;" > /dev/null 2>&1
        applied=$((applied + 1))
        echo -e "${GREEN}  Applied: $(basename "$migration_file")${NC}"
    fi
done

echo -e "${GREEN}✓ Database migration completed (applied: $applied, skipped: $skipped)${NC}"
